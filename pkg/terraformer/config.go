...
// StartUpdateWorker starts a worker goroutine, that will read from the state-update queue and call StoreState for
...
func (t *Terraformer) StartUpdateWorker() func() {
...
func (t *Terraformer) TriggerAndWaitForFinalStateUpdate() error {
	...
	select {
	case <-time.After(finalStateUpdateTimeout):
